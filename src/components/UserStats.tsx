...
import { BarChart3, History, Target, TrendingUp, Star, Award } from "lucide-react";
            ...
            <div className="space-y-6">
                ...
                >
                    ...
                    <div className="space-y-6">
                        ...
                        {(!safeStats.topicStats || safeStats.topicStats.length === 0) && (
                            <p className="text-foreground/40 text-center py-8 font-medium italic">Veri bulunamadı.</p>
                        )}
                    </div>
                </motion.div>
                <motion.div
                ...
                >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        {(safeStats.achievements || []).map((achievement: any, idx: number) => (
                            <motion.div
                                key={achievement.id}
                                initial={{ opacity: 0, scale: 0.8 }}
                                animate={{ opacity: 1, scale: 1 }}
                                transition={{ delay: idx * 0.1 }}
                                className="p-4 bg-foreground/5 rounded-2xl border border-border-theme flex flex-col items-center text-center group hover:border-secondary/30 transition-all"
                            >
                                ...
                                <h4 className="text-sm font-black text-foreground">{achievement.name}</h4>
                                <p className="text-[10px] text-foreground/40 font-medium mt-1 leading-tight">{achievement.description}</p>
                            </motion.div>
                        ))}
                        {(!safeStats.achievements || safeStats.achievements.length === 0) && (
                            <div className="col-span-full py-8 text-center bg-foreground/5 rounded-3xl border border-dashed border-border-theme">
                                <p className="text-foreground/40 text-sm font-medium italic">Henüz başarım kazanılmadı.</p>
                            </div>
                        )}
                    </div>
                </motion.div>
